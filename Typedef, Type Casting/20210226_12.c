...
serial number, name, surname, surname, position, length of service in years, 
salary in BGN, index to the structure, describing its boss. Write a program that 
introduces 10 employees, which are stored in an array of the described structures. 
The entry of employees can be done in two passes, the first entry of all data 
without the directory to the boss and the second pass, indicating to each employee 
who is his boss. Dynamic memory allocation and typedef must be used in the solution. */
